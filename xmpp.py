...
SENDER_ID = ''  # FCM SENDER_ID
SERVER_KEY = ''  # FCM SERVER KEY

logger = gcl.Client().logger(LOGGER_NAME)


def on_acknowledge(error, message_id, _from):
    if error:
...
def on_disconnect(draining):
    ...
    xmpp.connect(('fcm-preprod.googleapis.com', 5236), use_ssl=True)
...
def on_message(data):
    ...
    # logger.log_struct(msg) # TODO not use. google cloud logging 0.20 bug


logging.basicConfig(level=logging.DEBUG, format='%(levelname)-8s %(message)s')
logging.debug("Starting up")

xmpp = GCM(SENDER_ID + '@gcm.googleapis.com', SERVER_KEY)
xmpp.add_event_handler(XMPPEvent.CONNECTED, on_disconnect)
xmpp.add_event_handler(XMPPEvent.DISCONNECTED, on_session_start)
